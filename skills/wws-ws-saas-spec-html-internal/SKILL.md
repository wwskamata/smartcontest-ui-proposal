---
name: wws-ws-saas-spec-html-internal
description: |
...
以下の早見表で選定判断する。詳細な機能仕様・UI色が必要な場合のみ `references/products.md` を参照（トークン節約）。
...
| SNS/UGCキャンペーン | **SmartPRシリーズ**（詳細はPDF確認後） |
...
| プロダクト | 初期費用 | 月額 | 納期 |
|-----------|---------|------|------|
| SmartStampCard（スタンダード） | 25万円〜 | 3万円〜 | 10日 |
| SmartRecieco WEB（ライト） | 30万円〜 | 2万円〜 | 10日 |
| SmartStampRally | 要問合せ | 要問合せ | — |

**SmartStampCard 主要オプション：**
